...
describe("buildStageStatus — future stages reflect real notes (v3 — 2026-05-17)", () => {
  ...
  it("future stage with no notes → still pending (Rule 5 default)", () => {
    const cycle = activeCycle("learn");
    expect(buildStageStatus(cycle, NO_NOTES, SIG_ZERO).achieve).toBe("pending");
  });

  it("current stage on an active cycle stays in_progress even when completion gate satisfied", () => {
    // Reinforces Rule 1 — the orchestrator, not the dashboard, advances
    // the cycle's current_stage. Even when notes + signals would otherwise
    // satisfy isComplete(), the current stage stays in_progress.
