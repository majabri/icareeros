 ...
 *
 * Sprint 5 hotfix (2026-05-15) — Removed the prior `i <= currentIdx`
 * gate. Stages can now show their real status regardless of where the
 * cycle's `current_stage` index points. So if the user runs `/achieve`
 * directly while the cycle says current_stage='learn', the dashboard
 * will reflect the achievement instead of pretending Achieve is still
 * pending. The current stage on an ACTIVE cycle stays `in_progress`
 * (never auto-graduates to completed), because cycle advancement is
 * the orchestrator's job, not the dashboard's.
 *
 * Decision matrix (in order, first match wins):
 *
 *   1. cycle is active AND stage === current_stage  →  in_progress
 *      (preserved invariant — current stage of a live cycle never
