...
function LearnOutputPanel({ result }: { result: StoredLearn }) {
  // Sprint 5 hotfix (2026-05-16) — defensive normalize.
  const safeResult              = result as unknown as Record<string, unknown>;
  const resources               = arr<LearningResource>(safeResult.resources);
  const topSkillGaps            = arr<string>(safeResult.topSkillGaps);
  const summaryText             = str(safeResult.summary);
  const weeklyHoursNeeded       = num(safeResult.weeklyHoursNeeded);
  const estimatedCompletionWeeks = num(safeResult.estimatedCompletionWeeks);
  const sorted    = [...resources].sort(
    (a, b) => num((b as unknown as Record<string, unknown>).priorityScore)
            - num((a as unknown as Record<string, unknown>).priorityScore),
  );
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-5 border-l-4 border-l-brand-500">
        <div className="flex flex-wrap items-baseline justify-between gap-4">
          ...
          <div className="flex gap-6 text-right">
            <div>
              <div className="text-2xl font-bold tabular-nums text-brand-700">{weeklyHoursNeeded}</div>
              <div className="text-xs text-gray-500">hrs / week</div>
            </div>
            <div>
              <div className="text-2xl font-bold tabular-nums text-brand-700">{estimatedCompletionWeeks}</div>
...
function ResourceCard({
  ...
  const safeR         = r as unknown as Record<string, unknown>;
  const skillsCovered = arr<string>(safeR.skillsCovered);
  const titleText     = str(safeR.title);
  const provider      = str(safeR.provider);
  const estimatedHrs  = num(safeR.estimatedHours);
  const score         = Math.max(0, Math.min(100, num(safeR.priorityScore)));
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-gray-500 mt-0.5">
            {provider} · {estimatedHrs} hrs
          </p>
          {skillsCovered.length > 0 && (
            <div className="mt-2 flex flex-wrap gap-1">
              {skillsCovered.slice(0, 4).map((s) => (
                <AddSkillPill
                  key={s}
                  skill={s}
                  targetSkills={targetSkills}
              ...
              {skillsCovered.length > 4 && (
                <span className="text-[10px] text-gray-500 self-center">+{skillsCovered.length - 4}</span>
              )}
            </div>
          )}
