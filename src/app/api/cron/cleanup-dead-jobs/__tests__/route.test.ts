/**
 * fix/jobs-pipeline-crons — proxy route tests for cleanup-dead-jobs.
...
function reqWith(auth: string | undefined): Request {
  ...
  return new Request("http://localhost/api/cron/cleanup-dead-jobs", { method: "POST", headers });
}

describe("cron/cleanup-dead-jobs proxy route", () => {
  ...
  it("forwards to the edge function with the correct target URL", async () => {
    ...
    expect(url).toBe("https://x.supabase.co/functions/v1/cleanup-dead-jobs");
