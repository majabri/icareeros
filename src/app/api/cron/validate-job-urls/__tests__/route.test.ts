/**
 * fix/jobs-pipeline-crons — proxy route tests for validate-job-urls.
...
function reqWith(auth: string | undefined): Request {
  ...
  return new Request("http://localhost/api/cron/validate-job-urls", { method: "POST", headers });
}

describe("cron/validate-job-urls proxy route", () => {
  ...
  it("forwards to the edge function with the correct target URL", async () => {
    ...
    expect(url).toBe("https://x.supabase.co/functions/v1/validate-job-urls");
