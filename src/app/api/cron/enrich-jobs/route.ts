 ...
 * Function `enrich-jobs` which classifies ats_jobs into role_families / seniority_tier / extracted_skills / extracted_seniority. Runs every 4 hours at :30 offset from the ingest cron so we enrich the previous batch after ingest completes.
 *
...
export async function POST(req: NextRequest) {
  ...
  const target = `${supabaseUrl}/functions/v1/enrich-jobs`;
  ...
  try {
    ...
    let json: EnrichResponse | null = null;
    try { json = JSON.parse(text) as EnrichResponse; } catch { /* keep raw text */ }

    if (!res.ok) {
      console.error("[cron/enrich-jobs] edge function returned non-2xx:", res.status, text.slice(0, 500));
    ...
    console.info(
      `[cron/enrich-jobs] ok in ${elapsedMs}ms — processed=${json?.processed ?? 0} updated=${json?.updated ?? 0} errors=${json?.errors ?? 0}`,
    );
    return NextResponse.json({ ok: true, elapsedMs, result: json ?? null });
  } catch (e) {
    const msg = e instanceof Error ? e.message : "unknown";
    console.error("[cron/enrich-jobs] fetch failed:", msg);
