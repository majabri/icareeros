...
/** POST /api/admin/hire-users/[id]/reset-password — admin gate test. */
...
vi.mock("@supabase/supabase-js", () => ({
  createClient: () => ({
    ...
    auth: { admin: { getUserById: () => Promise.resolve({ data: { user: null }, error: null }), generateLink: () => Promise.resolve({ data: null, error: null }) } },
  }),
}));
...
describe("POST /api/admin/hire-users/[id]/reset-password", () => {
  it("403s for non-admins", async () => {
    const req = new Request("http://localhost/api/admin/hire-users/x/reset-password", { method: "POST" });
    const res = await POST(req, { params: Promise.resolve({ id: "emp-1" }) });
