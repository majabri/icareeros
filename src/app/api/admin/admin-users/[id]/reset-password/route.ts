/**
 * POST /api/admin/admin-users/[id]/reset-password — send a recovery email
 * to an admin user. Mirrors /api/admin/hire-users/[id]/reset-password but
 * verifies the target actually IS an admin (defense in depth — prevents
 * the wrong-tab footgun).
 */
...
const ROOT_URL = process.env.NEXT_PUBLIC_ROOT_URL ?? "https://icareeros.com";

export async function POST(
  _req: Request,
  { params }: { params: Promise<{ id: string }> },
) {
  const r = await requirePermission("users.change_plan"); // any L2+ admin can reset
  ...
  // Confirm target actually has admin privileges (wrong-tab safety).
  const { data: prof } = await svc
    .from("profiles")
    .select("role, admin_role")
    .eq("user_id", id)
    .maybeSingle();
  const isAdmin = Boolean((prof as { admin_role?: string | null } | null)?.admin_role)
              || (prof as { role?: string | null } | null)?.role === "admin";
  if (!isAdmin) {
