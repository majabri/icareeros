...
export default async function AdminUsersPage() {
  ...
  const [
    ...
    svc.from("profiles").select("user_id, email, full_name, created_at").order("created_at", { ascending: false }).limit(200),
      ...
      <div className="overflow-x-auto rounded-xl border border-gray-200 bg-white shadow-sm">
        <table className="min-w-full divide-y divide-gray-100 text-sm">
          <thead className="bg-gray-50">
            <tr>
              {["User","Plan","Cycles","Joined","Supabase","Actions"].map(h => (
            ...
            {users.map(u => (
              <tr key={u.user_id} className="hover:bg-gray-50 transition-colors">
                <td className="px-4 py-3">
                  ...
                  {u.full_name && <p className="text-xs text-gray-400 truncate max-w-[200px]">{u.full_name}</p>}
                </td>
                <td className="px-4 py-3">
                  <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${planBadgeClass(u.plan)}`}>{u.plan}</span>
                  {u.plan_status !== "active" && <span className="ml-1 text-xs text-red-400">{u.plan_status}</span>}
                </td>
                ...
                <td className="px-4 py-3">
                  <a href={`https://supabase.com/dashboard/project/kuneabeiwcxavvyyfjkx/auth/users?search=${u.email ?? u.user_id}`}
                    target="_blank" rel="noopener noreferrer"
                    className="text-xs text-blue-600 hover:text-blue-800 font-medium">View ↗</a>
                </td>
                <td className="px-4 py-3">
                  <AdminUserActions userId={u.user_id} currentPlan={u.plan}
                    email={u.email ?? u.user_id} emailConfirmed={u.email_confirmed} />
                </td>
              </tr>
            ))}
          </tbody>
