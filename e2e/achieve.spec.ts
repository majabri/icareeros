...
test.beforeAll(async ({ browser }) => {
  ...
  try {
    ...
    const routeRes = await page.request.post("/api/career-os/achieve", {
      data: { cycle_id: "probe" },
...
test("POST /api/career-os/achieve → 400 when cycle_id is missing", async ({ browser }) => {
  ...
  try {
    ...
    const res = await page.request.post("/api/career-os/achieve", {
      data: {},
      headers: { "Content-Type": "application/json" },
      failOnStatusCode: false,
    });
...
test("Dashboard shows Achieve stage card", async ({ browser }) => {
